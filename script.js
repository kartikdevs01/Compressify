...
window.addEventListener('DOMContentLoaded', () => {
  /* Resume Builder is intentionally local-only; no personal data is sent to a server. */
  const resumeAuth = $('#resumeAuth'), resumeDashboard = $('#resumeDashboard'), signupForm = $('#signupForm'), loginForm = $('#loginForm'), resumeForm = $('#resumeForm');
  const savedList = $('#resumeSavedList'), welcomeName = $('#welcomeName'), editorTitle = $('#resumeEditorTitle'), toast = $('#resumeToast');
  const accountKey = 'compressify_ai_account', sessionKey = 'compressify_ai_session', resumesKey = 'compressify_ai_resumes'; let activeResumeId = null, toastTimer;
  const getAccount = () => JSON.parse(localStorage.getItem(accountKey) || 'null'); const getResumes = () => JSON.parse(localStorage.getItem(resumesKey) || '[]'); const setResumes = list => localStorage.setItem(resumesKey, JSON.stringify(list));
  const notify = (message, error = false) => { toast.textContent = message; toast.classList.toggle('error', error); toast.classList.add('show'); clearTimeout(toastTimer); toastTimer = setTimeout(() => toast.classList.remove('show'), 3200); };
  const defaultResume = () => ({ id: crypto.randomUUID ? crypto.randomUUID() : String(Date.now()), title: 'Untitled Resume', name: '', headline: '', email: '', phone: '', location: '', summary: '', experienceRole: '', company: '', dates: '', experience: '', skills: '', coverLetter: '', template: 'classic' });
  function value(data, name) { return data[name] || ''; }
  function fillForm(data) { Object.keys(data).forEach(key => { const field = resumeForm.elements[key]; if (field) field.value = data[key]; }); editorTitle.textContent = data.title || 'Untitled Resume'; updatePreview(); }
  function formData() { return Object.fromEntries(new FormData(resumeForm).entries()); }
  function updatePreview() { const data = formData(); editorTitle.textContent = data.title || 'Untitled Resume'; document.querySelectorAll('[data-preview]').forEach(node => { const key = node.dataset.preview; node.textContent = value(data, key) || node.textContent; }); const list = $('[data-preview-list="experience"]'); const lines = data.experience.split('\n').map(line => line.trim()).filter(Boolean); list.replaceChildren(...(lines.length ? lines : ['Add your achievements to show results.']).map(line => { const li = document.createElement('li'); li.textContent = line; return li; })); const preview = $('#resumePreview'); preview.className = `resume-preview template-${data.template || 'classic'}`; }
  function renderSavedList() { const resumes = getResumes(); savedList.replaceChildren(); resumes.forEach(resume => { const item = document.createElement('button'); item.type = 'button'; item.className = `saved-resume ${resume.id === activeResumeId ? 'active' : ''}`; item.innerHTML = `<strong>${resume.title || 'Untitled Resume'}</strong><span>${resume.headline || 'Resume draft'}</span>`; item.addEventListener('click', () => { activeResumeId = resume.id; fillForm(resume); renderSavedList(); }); savedList.append(item); }); }
  function showDashboard() { const account = getAccount(); if (!account) return; welcomeName.textContent = `Hi, ${account.name.split(' ')[0]}`; resumeAuth.classList.add('hidden'); resumeDashboard.classList.remove('hidden'); const resumes = getResumes(); if (resumes.length) { activeResumeId = resumes[0].id; fillForm(resumes[0]); } else { const draft = defaultResume(); activeResumeId = draft.id; fillForm(draft); } renderSavedList(); }
  function saveResume(showMessage = true) { const data = { ...formData(), id: activeResumeId || defaultResume().id }; activeResumeId = data.id; const resumes = getResumes(), index = resumes.findIndex(item => item.id === data.id); if (index >= 0) resumes[index] = data; else resumes.unshift(data); setResumes(resumes); renderSavedList(); if (showMessage) notify('Resume saved securely in this browser.'); return data; }
  function newResume() { const draft = defaultResume(); activeResumeId = draft.id; resumeForm.reset(); fillForm(draft); renderSavedList(); notify('New resume ready to edit.'); }
  function switchAuth(view) { document.querySelectorAll('.auth-tab').forEach(button => button.classList.toggle('active', button.dataset.authView === view)); signupForm.classList.toggle('hidden', view !== 'signup'); loginForm.classList.toggle('hidden', view !== 'login'); }
  document.querySelectorAll('.auth-tab').forEach(button => button.addEventListener('click', () => switchAuth(button.dataset.authView)));
  signupForm.addEventListener('submit', event => { event.preventDefault(); const name = $('#signupName').value.trim(), email = $('#signupEmail').value.trim().toLowerCase(), password = $('#signupPassword').value; if (password !== $('#signupConfirm').value) return notify('Passwords do not match.', true); localStorage.setItem(accountKey, JSON.stringify({ name, email, password })); localStorage.setItem(sessionKey, email); showDashboard(); notify('Your private workspace is ready.'); });
  loginForm.addEventListener('submit', event => { event.preventDefault(); const account = getAccount(); if (!account || account.email !== $('#loginEmail').value.trim().toLowerCase() || account.password !== $('#loginPassword').value) return notify('Email or password is incorrect on this device.', true); localStorage.setItem(sessionKey, account.email); showDashboard(); notify('Welcome back.'); });
  resumeForm.addEventListener('input', updatePreview); $('#resumeTemplate').addEventListener('change', updatePreview); $('#saveResumeButton').addEventListener('click', () => saveResume()); $('#newResumeButton').addEventListener('click', newResume); $('#duplicateResumeButton').addEventListener('click', () => { const copy = { ...saveResume(false), id: defaultResume().id, title: `${formData().title || 'Untitled Resume'} Copy` }; activeResumeId = copy.id; fillForm(copy); saveResume(false); notify('Resume duplicated.'); });
  $('#logoutButton').addEventListener('click', () => { localStorage.removeItem(sessionKey); resumeDashboard.classList.add('hidden'); resumeAuth.classList.remove('hidden'); switchAuth('login'); });
  document.querySelectorAll('[data-ai]').forEach(button => button.addEventListener('click', () => { const data = formData(), mode = button.dataset.ai; if (mode === 'summary') resumeForm.elements.summary.value = `${data.headline || 'Results-focused professional'} with experience in ${data.skills || 'collaboration and problem solving'}. Known for turning complex goals into clear, measurable outcomes.`; if (mode === 'bullets') resumeForm.elements.experience.value = [`Led high-impact initiatives as ${data.experienceRole || 'a specialist'}, improving quality and delivery.`, `Partnered with cross-functional teams at ${data.company || 'the organization'} to solve customer needs.`, 'Used data and feedback to prioritize work and deliver measurable results.'].join('\n'); if (mode === 'cover') resumeForm.elements.coverLetter.value = `Dear Hiring Manager,\n\nI am excited to apply for the ${data.headline || 'open'} role. My experience in ${data.skills || 'creating thoughtful solutions'} and focus on meaningful outcomes would allow me to contribute from day one.\n\nSincerely,\n${data.name || 'Your Name'}`; updatePreview(); notify('Smart writing suggestion added—review and personalise it.'); }));
  function wrapText(text, font, size, width) { const words = text.split(/\s+/), lines = []; let line = ''; words.forEach(word => { const next = `${line} ${word}`.trim(); if (font.widthOfTextAtSize(next, size) > width && line) { lines.push(line); line = word; } else line = next; }); if (line) lines.push(line); return lines; }
  $('#exportResumeButton').addEventListener('click', async () => { const data = saveResume(false); if (!window.PDFLib) return notify('PDF export is loading. Please try again.', true); try { const { PDFDocument, StandardFonts, rgb } = window.PDFLib, pdf = await PDFDocument.create(), page = pdf.addPage([595, 842]), regular = await pdf.embedFont(StandardFonts.Helvetica), bold = await pdf.embedFont(StandardFonts.HelveticaBold); let y = 790; const draw = (text, font = regular, size = 10, color = rgb(.15,.12,.2)) => { wrapText(text || '', font, size, 490).forEach(line => { if (y < 45) return; page.drawText(line, { x: 52, y, size, font, color }); y -= size + 5; }); }; draw(data.name || 'Your Name', bold, 25, rgb(.27,.18,.55)); draw(data.headline, bold, 12); draw([data.email, data.phone, data.location].filter(Boolean).join('  •  ')); y -= 10; [['PROFILE', data.summary], ['EXPERIENCE', `${data.experienceRole}${data.company ? ` — ${data.company}` : ''}${data.dates ? ` (${data.dates})` : ''}\n${data.experience}`], ['SKILLS', data.skills], ['COVER LETTER', data.coverLetter]].forEach(([heading, body]) => { if (!body) return; draw(heading, bold, 11, rgb(.44,.34,.8)); draw(body); y -= 8; }); const bytes = await pdf.save(), url = URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })), link = document.createElement('a'); link.href = url; link.download = `${(data.name || 'resume').replace(/[^a-z0-9]+/gi, '-').toLowerCase()}.pdf`; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); notify('Your ATS-friendly PDF resume is ready.'); } catch { notify('PDF export could not be completed. Please try again.', true); } });
  if (localStorage.getItem(sessionKey) && getAccount()) showDashboard();
});
